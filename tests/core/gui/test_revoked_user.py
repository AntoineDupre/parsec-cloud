...
async def logged_gui(
    aqtbot, gui_factory, running_backend, autoclose_dialog, core_config, bob, alice_user_fs
):
    ...
    assert not gui.test_get_devices_widget().initialized
    assert gui.test_get_users_widget().users == []
    assert not gui.test_get_users_widget().initialized
...
async def test_revoked_notification(
    ...
    async with aqtbot.wait_signals([central_widget.new_notification]):
